	...
	function _upload_files( $post_id = 0 ) {
		...
		for ( $i = 0; $i < count( $files['name'] ); $i++ ) {
			...
			$post_overrides = array(
				...
				'post_title' => urld_clean($_POST['post_title']),
...
// global $wpdb;
// $query = "SELECT * FROM `wp_wpgmza` WHERE 1=1";
// $result = $wpdb->get_results ($query);
// foreach ( $result as $print ) {
// 	if(strpos(!$print->pic, "http://")) {
// 		$query1 = "SELECT * FROM `wp_posts` WHERE `post_title` = '".$print->pic."'";
// 		$result1 = $wpdb->get_results($query1);
// 		$query2 = "UPDATE `wp_wpgmza` SET `pic` = '".$result1[0]->guid."' WHERE `pic` = '".$print->pic."'";
